...
export default function OpenEvents({navigation}: any) {
    const windowWidth = Dimensions.get('window').width;
    ...
    useFocusEffect(
      React.useCallback(() => {
        ...
        getCurrentUser(db).then((user: User)=>{
          getEvents(db,user.id,0).then((templates: Event[]) => {
            setMyArray(shuffleExceptFirst(templates.filter(element => element.isTemplate == 0 && (element.date.flag == 1 || element.time.flag == 1 || element.location.flag == 1 || element.participants.flag == 1 || element.note.flag == 1))))
          })
        })}, [])
    );
    ...
    return (
      <View style={{ flex: 1, backgroundColor:'#ffffff'}}>
          <TopNavigation navigation={navigation} title={'Open Events'} color={'#000000'} />
