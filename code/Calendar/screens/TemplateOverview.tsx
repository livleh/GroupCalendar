...
const EmojiIconA = ({emoji, color}: any) => {
  return(
    <View style={{backgroundColor:color, width:4.5 * marginwindowWidth, height:4.5 * marginwindowWidth, justifyContent:'center', alignItems:'center',borderRadius:8, marginRight:5}}>
      <Ionicons name={emoji} style={{fontSize:2.5 * marginwindowWidth}}/> 
    </View>
  )
}
...
export default function TemplateOverview({route, navigation}: any) {
      ...
      React.useCallback(() => {
        ...
        getCurrentUser(db).then((user: User)=>{
          getEvents(db,user.id,1).then((templates: Event[]) => {
            setMyArray(shuffleExceptFirst(templates));
    ...
    const renderItemA = ({item}: {item: Event}) => {
      ...
      return (
        (item.id == "DUMMYTemplate" && item.isTemplate == 1) 
        ? (<TouchableOpacity onPress={()=>navigation.navigate('NewNewEntry', {id: item.id, currentEvent: dummyTemplate, isTemplate: item.isTemplate})} style={{marginVertical:marginwindowWidth}}>
            <HalfLengthTile>
    ...
    const renderItemB = ({item}: {item: Event}) => {
      ...
      return (
        (item.id == "DUMMYTemplate" && item.isTemplate == 1) 
        ? (<TouchableOpacity onPress={()=>navigation.navigate('NewNewEntry', {id: item.id,currentEvent: dummyTemplate, isTemplate: item.isTemplate, editing: 0})} style={{marginVertical:10}}>
            <FullLengthTile>
              <View style={{justifyContent:'center', alignContent:'center', display:'flex',flex: 1 }} >
    ...
    return (
      <View style={{ flex: 1, backgroundColor:'#ffffff'}}>
          ...
          <SafeAreaView style={{marginVertical:'5%', flex:1}}>
            ...
            <TouchableOpacity
                onPress={async () =>{
                    navigation.navigate("NewNewEntry", {id: "DUMMYTemplate", currentEvent: dummyTemplate, isTemplate: 1, editing:0, toptitle:'Create Template'})
                }}                style={{
                borderWidth: 1,
                borderColor: '#000000',
                alignItems: 'center',
        ...
        >
          <SafeAreaView style={{flex: 1, backgroundColor: 'white'}}>
            ...
            <TouchableOpacity onPress={() => {setModalVisible(false); navigation.navigate("NewNewEntry", {currentEvent: currentEvent!, isTemplate: 1, editing: false})}} style={{backgroundColor: "black", borderRadius: 20, width: 200, alignSelf: 'center', marginBottom: 10}}><Text style={{color: 'white', textAlign: 'center', fontSize: 17, padding: 10}}>Use this template</Text></TouchableOpacity>
        ...
        >
          <View style ={{flex:1, backgroundColor: 'rgba(0, 0, 0, 0.5)', justifyContent: 'center'}}>
            <View style={{backgroundColor: "white", borderRadius: 10, padding: 20, marginHorizontal: 50}}>
              <Text style={{fontSize: 15}}>Do you really want to delete template "{currentEvent?.title}"?</Text>
